...
function badge(text: string, color = '#3b82d4'): string {
...
export function renderWelcome(v: WelcomeVars): RenderedEmail {
  const subject = 'Welcome to Airbus Tools';
  const html = wrap(
    h1('Welcome aboard, ' + v.recipientName + '!') +
      ...
      p('If you have any questions, reach out to our support team at any time.'),
    subject,
  );
  const text =
    `Welcome to Airbus Tools, ${v.recipientName}!\n\n` +
...
export function renderOrderRequest(v: OrderRequestVars): RenderedEmail {
  const subject = `Order Request Received — ${v.orderNumber}`;
  const html = wrap(
    h1('Order Request Submitted') +
      ...
      p("We'll notify you as soon as the supplier responds."),
    subject,
  );
  const text =
...
export function renderOrderAccepted(v: OrderAcceptedVars): RenderedEmail {
  const subject = `Order Accepted — ${v.orderNumber}`;
  const html = wrap(
    h1('Your Order Has Been Accepted') +
      ...
      p('Proceed to payment to complete your order.'),
    subject,
  );
  const text =
...
export function renderOrderRejected(v: OrderRejectedVars): RenderedEmail {
  const subject = `Order Rejected — ${v.orderNumber}`;
  const html = wrap(
    h1('Your Order Could Not Be Fulfilled') +
      ...
      p('You may submit a new order at any time. Contact support if you need assistance.'),
    subject,
  );
  const text =
...
export function renderPaymentConfirmation(v: PaymentConfirmationVars): RenderedEmail {
  ...
  const html = wrap(
    h1('Payment Confirmed') +
      ...
      p('A receipt has been attached to your account. Thank you for your business.'),
    subject,
  );
  const text =
...
export function renderPasswordChanged(v: PasswordChangedVars): RenderedEmail {
  const subject = 'Your Password Was Changed';
  const html = wrap(
    h1('Password Changed Successfully') +
      p(`Hi ${v.recipientName}, your Airbus Tools password was changed on ${v.changedAt}.`) +
      p('If you did not make this change, please contact support immediately and reset your password.'),
    subject,
  );
  const text =
...
export function renderNotification(v: NotificationEmailVars): RenderedEmail {
  const html = wrap(
    h1(v.subject) +
