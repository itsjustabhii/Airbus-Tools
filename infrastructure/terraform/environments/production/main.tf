...
# Region: us-east-1  |  Atlas tier: M30  |  Worker: Fargate Spot + base-1 Fargate
###############################################################################
...
variable "domain_name" {
  description = "Base production domain (e.g. airbus-tools.example.com). Required — no default."
  type        = string
}
...
module "atlas" {
  ...
  enable_vpc_peering     = true  # VPC peering enabled in production
...
module "cache" {
  ...
  node_type          = "cache.r6g.large"  # memory-optimised for production
